...
`;
    ...
    /** 在聊天界面右下角创建头像设置浮动按钮 + 面板（不依赖设置页）。 */
    function createFloatingPanel(controller) {
      if (document.getElementById("dsh-chat-avatar-fab") !== null) return () => {};

      const fab = document.createElement("button");
      fab.id = "dsh-chat-avatar-fab";
      fab.textContent = "👤";
      fab.title = "聊天头像设置";
      fab.setAttribute("aria-label", "聊天头像设置");
      fab.style.cssText =
        "position:fixed;right:20px;bottom:20px;z-index:2147483000;" +
        "width:48px;height:48px;border-radius:50%;border:none;" +
        "background:linear-gradient(135deg,#4d6bfe,#7c3aed);color:#fff;" +
        "font-size:22px;cursor:pointer;box-shadow:0 4px 14px rgba(0,0,0,.25);" +
        "display:flex;align-items:center;justify-content:center;";

      const panel = document.createElement("div");
      panel.id = "dsh-chat-avatar-panel";
      panel.style.cssText =
        "position:fixed;right:20px;bottom:76px;z-index:2147483000;width:300px;" +
        "background:var(--dsw-alias-bg-base,#fff);border:1px solid var(--dsw-alias-border-l1,#ddd);" +
        "border-radius:12px;box-shadow:0 8px 30px rgba(0,0,0,.2);padding:14px;display:none;";

      const title = document.createElement("div");
      title.textContent = "聊天头像设置";
      title.style.cssText = "font-size:15px;font-weight:600;margin-bottom:12px;color:var(--dsw-alias-label-primary,#111);";

      const labelStyle = "display:block;font-size:12px;margin:8px 0 4px;color:var(--dsw-alias-label-secondary,#666);";
      const inputStyle =
        "box-sizing:border-box;width:100%;padding:8px 10px;font-size:13px;line-height:18px;" +
        "border:1px solid var(--dsw-alias-border-l1,#ccc);border-radius:8px;" +
        "background:var(--dsw-alias-bg-base,#fff);color:var(--dsw-alias-label-primary,#111);outline:none;";

      const userLabel = document.createElement("label");
      userLabel.textContent = "用户头像 URL（右侧）";
      userLabel.style.cssText = labelStyle;
      const userInput = document.createElement("input");
      userInput.type = "text";
      userInput.placeholder = "留空使用本地 avatars/user.* 或默认头像";
      userInput.style.cssText = inputStyle;

      const aiLabel = document.createElement("label");
      aiLabel.textContent = "AI 头像 URL（左侧）";
      aiLabel.style.cssText = labelStyle;
      const aiInput = document.createElement("input");
      aiInput.type = "text";
      aiInput.placeholder = "留空使用本地 avatars/ai.* 或默认头像";
      aiInput.style.cssText = inputStyle;

      const hint = document.createElement("div");
      hint.textContent = "支持 https:// 图片链接；留空则使用本地 avatars 文件夹或默认头像。";
      hint.style.cssText = "font-size:11px;color:var(--dsw-alias-label-tertiary,#999);margin:8px 0 12px;line-height:16px;";

      const status = document.createElement("div");
      status.style.cssText = "font-size:12px;color:#16a34a;min-height:16px;margin-bottom:8px;";
      ...
      saveBtn.style.cssText =
        "padding:7px 14px;font-size:13px;border-radius:8px;border:none;cursor:pointer;" +
        "background:var(--dsw-alias-button-info-fill,#4d6bfe);color:#fff;";
      ...
      const fillFromSnapshot = () => {
        try {
          const s = controller.scope.getSnapshot();
          if (!s?.value) return;
          userInput.value = typeof s.value.userAvatar === "string" ? s.value.userAvatar : "";
          aiInput.value = typeof s.value.assistantAvatar === "string" ? s.value.assistantAvatar : "";
        } catch {}
      };
      ...
      saveBtn.addEventListener("click", async () => {
        ...
        const a = aiInput.value.trim();
        saveBtn.disabled = true;
        saveBtn.textContent = "保存中…";
        try {
          const actions = controller.inject();
          await actions.save(u, a);
          applyRemoteConfig({ userAvatar: u, assistantAvatar: a });
      ...
      panel.append(title, userLabel, userInput, aiLabel, aiInput, hint, status, buttons);
      buttons.append(closeBtn, saveBtn);
    ...
    function apply(ctx) {
      ...
      createFloatingPanel(controller);
