...
// - 注册「设置 → 插件 → 插件配置」卡片，并实时应用保存后的头像 URL。
        ...
        inject() {
          return {
            ...
            async save(userAvatar, assistantAvatar) {
              const user = String(userAvatar ?? "").trim();
              const assistant = String(assistantAvatar ?? "").trim();
              if (user === "") await scope.unset("userAvatar");
              else await scope.set("userAvatar", user);
              if (assistant === "") await scope.unset("assistantAvatar");
              else await scope.set("assistantAvatar", assistant);
              return true;
            },
          };
        },
